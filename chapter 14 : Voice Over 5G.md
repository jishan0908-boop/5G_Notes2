...
* Once this signaling has been establish on the bearer IMS signaling  , and once all the sufficent information are get then the IMS Media Bearer establishment take place , over it the actual voice communication will take place , once this is set-up an update SDP Offer msg will be send form the Calling UE to the Called UE, to Induicates media bearer has been established , then it will be response by the 200 OK msg , then a 180 Ringing msg would recive , by the calling UE , that the called UE is now hearing the ringing sound , and ringing tone would also be genrated to the calling ue , once Called UE picks up the phone , it send the 200 OK msg , and calling UE will acknowledege this ,and the calling is began.

# IMS Media Bearer Establishment:
* We know that the P-CSCF is the first point of contact of the UE with the IMS system , when there is a voice call the P-CSCF will be able to monitor the SIP messages , in those SIP msges there is SDP , which describes the IP addresses , codecs , and ports , in order to set-up the voice call .So , based upon the negoiative parameter between calling and the called party , then P-CSCF sends the msg to the PCF , it is the part of the 5G Core network system , so P-CSCF will use the Npcf_PolicyAuthorization_Create service in porder to communicate with the , PCF and the P-CSCF , would also inform PCF about the parameter, based upon which the voice call , would be set-up , PCF selects the UPF to Establish QoS flow for IMS Bearer , instructs Corresponding SMF, then SMF will coordinate with the AMF and also with the UPF in order to set-up the QoS in order to set-up the IMS Bearer 

# Tearing Down the Call
* Now we will see how 5G call is finished , at the SIP level, UE sends msg by the SIP to another UE response by the 200 OK message , then at the SIP level this voice call finish
* So the BYE nd OK msg are send by the P-CSCF , so it comes to know that the call has terminated , then it uses the service Npcf_PolicyAuthorization_Delete, in ordr to inform the PCF that it has to tear down this QoS flow , and PCF , cannot itself tear down this Qos flow , rather it would ask the SMF , in order to do so , the SMF does that to tear down the QoS flow.
